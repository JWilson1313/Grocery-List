...
function logNames(items) {
  // TODO: use `forEach`
  INVENTORY.forEach((name) => console.log(name.name));
}
...
function getUppercaseNames(items) {
  // TODO: use `map`
  return items.map((itemId) => itemId.name.toUpperCase());
}
...
function getItemsByCategory(items, category) {
  // TODO: use `filter`
  const Category = INVENTORY.filter((category) => "");
}
/**
 * @param {Item[]} items - array of items
...
uantity - number of this item in inventory
 */
